...
def extract_data(invoicefile, templates=None, input_module=pdftotext):
    ...
    tables = camelot.read_pdf(invoicefile)

    df = tables[0].df
    with open('d:\\attachment\\foo.json', 'w', encoding='utf-8') as file:
...
#        nltk.download('punkt')
    ...
    # lang = detect(extracted_str)
    translator = Translator()
    lang = translator.detect(extracted_str)
    if lang.lang == 'th':
        translation = translator.translate(extracted_str, dest='en')

        # extracted_str_list = extracted_str.split('\n')
        # for s in extracted_str_list:
        #    s = word_tokenize(s, keep_whitespace=False)
        ner = ThaiNameTagger()
        ner.get_ner(extracted_str)
        issuer = ''
        for item in ner.sent_ner:
            if (item[1] == 'B-ORGANIZATION' or item[1] == 'I-ORGANIZATION'):
                issuer = issuer + item[0]

    if (lang.lang == 'en'):

 """       sn = StanfordNERTagger('D:\\Robot\\Stanford\\english.all.3class.distsim.crf.ser.gz',
                               path_to_jar='D:\\Robot\\Stanford\\stanford-ner-2018-10-16\\stanford-ner.jar')
        ner_tagged_sentences = sn.tag(extracted_str.split())
        named_entities = []
    ...
    #        translation = translator.translate(extracted_str, dest='th')
    #        model = 'd:\\robot\\bettybot\\'
    #        model = 'en_core_web_lg'
    #        nlp = spacy.load(model)
    #        e_str = extracted_str.replace("\r","")
    #        delimiters = "\n", "  "
    #        regexPattern = '|'.join(map(re.escape, delimiters))
    #        extracted_str_list = e_str.split('\n')
    #        st = re.split(regexPattern, e_str)
    #        st = list(filter(None, st))
    #        for s in st:
    #            doc = nlp(s)
    #            logger.debug(doc.ents)
